...
async function ensureSession() {
  ...
  try {
    ...
    if (active && !active.endedAt) {
      // a mock in flight is offered as "Continue" on the Mocks page — never auto-open the exam runner
      if (active.kind !== "mock" && (await study.resume(active.id))) return;
    }
...
<template>
  <div class="flex min-h-dvh flex-col">
    ...
    <StudyIdle
      ...
      :ready="!session"
      :can-resume="!!study.activeSession.value && !study.activeSession.value.endedAt && study.activeSession.value.kind !== 'mock'"
