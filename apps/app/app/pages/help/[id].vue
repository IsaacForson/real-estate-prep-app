...
<template>
  <div class="max-w-3xl mx-auto safe-px py-6 md:py-12 grid gap-6 anim-fade-up">
    <NuxtLink to="/help" class="inline-flex items-center gap-1 text-sm text-accent font-medium"><Icon name="arrow-left" :size="16" />Help Center</NuxtLink>
    <template v-if="article">
      <header class="grid gap-2">
        <p class="eyebrow">{{ article.section ?? 'General' }}</p>
        <h1 class="text-3xl display">{{ article.title }}</h1>
      </header>
      <article class="rich text-[16px] text-ink-2 leading-relaxed grid gap-3">
      ...
      <AppCard tone="paper">
        <div class="flex flex-wrap items-center gap-3">
          <p class="flex-1 text-sm text-ink-2">Didn't answer it? A person will.</p>
      ...
      <section v-if="related.length" class="grid gap-2">
        ...
        <NuxtLink v-for="a in related" :key="a.id" :to="`/help/${a.id}`" class="rounded-card bg-surface border border-line px-4 py-3 flex items-center gap-3 hover:bg-surface-2"><span class="flex-1 font-medium text-[15px]">{{ a.title }}</span><Icon name="chevron-right" :size="18" class="text-muted" /></NuxtLink>
      </section>
    </template>
    <Skeleton v-else-if="!help.loaded.value" :lines="6" />
    <EmptyState v-else icon="help" title="Article not found" body="It may have moved. Search the help center instead."><AppButton to="/help" variant="primary" size="sm">Search</AppButton></EmptyState>
  </div>
</template>
